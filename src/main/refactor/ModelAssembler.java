...
/**
 * Construye la jerarquía Workspace→Project→Class→Method rellenando todas las
 * métricas y planes de extracción.
 */
public final class ModelAssembler {
	...
	public WorkspaceMetrics buildWorkspace(IWorkspaceRoot root) {
		...
		return WorkspaceMetrics.builder().name(root.getLocation().lastSegment()).analysisDate(LocalDate.now())
				.projects(projects).build();
	}

	public ProjectMetrics buildProject(IProject project) throws CoreException {
		...
		return ProjectMetrics.builder().name(project.getName()).classes(classes).build();
	}

	public ClassMetrics buildClass(ICompilationUnit icu) {
		...
		cu.accept(new ASTVisitor() {
			...
			public boolean visit(MethodDeclaration node) {
				...
				try {
					methods.add(extractor.analyseAndPlan(cu, node, currentCc, currentLoc));
				} catch (CoreException e) {
					e.printStackTrace();
				}
		...
		return ClassMetrics.builder().name(icu.getElementName().replace(".java", "")).currentLoc(currentLocTotal)
				.refactoredLoc(refactoredLocTotal).methods(methods).build();
	}
	...
	private <T> int sum(List<T> list, ToIntFunction<T> mapper) {
